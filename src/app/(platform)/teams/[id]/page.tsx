...
export default async function TeamDetailPage({
  ...
  const members = [
    ...(team.leader && !leaderInList
      ? [{ id: `leader-${team.leader.id}`, session_in_team: team.leader.session ?? [], user: team.leader as MemberCardData, isLeader: true }]
      : []),
    ...team.team_members.map(tm => ({
      id: tm.id, session_in_team: tm.session_in_team ?? [],
      user: tm.user, isLeader: tm.user.id === team.leader_id,
    })),
  ]
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '8px' }}>
          ...
          {canEdit && (
            <Link href={`/teams/${id}/edit`} style={{
              ...
              border: '1px solid #d1d5db', background: '#fff', textDecoration: 'none', color: '#374151', flexShrink: 0,
            }}>
              수정
            </Link>
          )}
